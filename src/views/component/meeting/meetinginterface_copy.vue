...
import { MemberInfo } from '@/utils/livekit'
...
const liveKitManager = new LiveKitManager({},memberList,import.meta.env.VITE_WS_URL)
...
onMounted(async () => {
    window.ipcRenderer.once('initData', (event, data : any) => {
        ...
        liveKitManager.connectToRoom(import.meta.env.VITE_WS_URL,data.token).then(() => {
            liveKitManager.setScreenShare(isVideoOn.value,isVoiceOn.value).then(() => {
                const data = liveKitManager.getMetaData()
                Object.assign(meetingInfo,data)
            })
        })
